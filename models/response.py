...
class ResponseBase(SQLModel):
    ...
    answer: Dict[str, Any] = Field(
        ...
        description="User answer data (JSON)"
    )
...
class Response(ResponseBase, table=True):
    ...
    user_id: int | None = Field(default=None, foreign_key="user.id", description="User ID (if authenticated)")
...
class ResponseSummary(SQLModel):
    ...
    response_data: Dict[str, Any] = Field(
        sa_column=Column(JSON),
        description="Aggregated response statistics"
    )
