...
from models import (
    ...
    Question, Survey, UserData
)
from schemas import SuccessResponse, ErrorResponse, UserSessionData
...
async def create_response(
    ...
    try:
        ...
        if not _validate_response_data(question, response_data.answer):
            raise HTTPException(
                status_code=400,
                detail="Invalid response data for question type"
            )
...
async def get_responses_by_question(
    ...
    try:
        ...
        if not question.survey.is_public:
            raise HTTPException(
                status_code=403,
                detail="Cannot access responses for private survey"
            )
...
async def get_responses_by_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch user responses: {str(e)}"
        )
...
async def get_survey_progress(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch survey progress: {str(e)}"
        )
...
async def validate_response_before_save(
    ...
    try:
        ...
        return {
            ...
            "validation_message": _get_validation_message(question, answer) if not is_valid else None
        }
