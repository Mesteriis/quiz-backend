...
async def get_vapid_public_key():
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Ошибка получения VAPID ключа: {str(e)}",
        )
...
async def unsubscribe_from_push_notifications(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка удаления подписки: {str(e)}",
        )
...
async def update_notification_categories(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка обновления категорий: {str(e)}",
        )
...
async def record_notification_analytics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка записи аналитики: {str(e)}",
        )
...
async def get_user_subscriptions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка получения подписок: {str(e)}",
        )
...
async def cleanup_expired_subscriptions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка очистки подписок: {str(e)}",
        )
