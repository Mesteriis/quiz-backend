...
async def generate_survey_pdf_report(
    ...
    try:
        ...
        responses_result = await session.execute(responses_query, {"survey_id": survey_id})
        ...
        analytics_result = await session.execute(analytics_query, {"survey_id": survey_id})
        ...
        analytics_data = {
            ...
            "first_response": analytics_raw.first_response.isoformat() if analytics_raw.first_response else None,
            "last_response": analytics_raw.last_response.isoformat() if analytics_raw.last_response else None
        }
        
        # Generate PDF
        pdf_bytes = pdf_service.generate_survey_report(survey_data, responses_data, analytics_data)
        ...
        raise HTTPException(
            ...
            detail=f"Failed to generate survey PDF report: {str(e)}"
        )
...
async def generate_user_pdf_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to generate user PDF report: {str(e)}"
        )
...
async def generate_my_responses_pdf_report(
    ...
    try:
        ...
        responses_result = await session.execute(responses_query, {"user_id": current_user.id})
        responses_raw = responses_result.fetchall()
        ...
        return Response(
            ...
            media_type="application/pdf",
            headers={
                "Content-Disposition": f"attachment; filename=my_responses_report.pdf"
            }
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to generate your responses PDF report: {str(e)}"
        ) 