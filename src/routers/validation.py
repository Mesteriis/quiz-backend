...
async def validate_email_endpoint(
    request: EmailValidationRequest, session: AsyncSession = Depends(get_async_session)
):
    """
...
async def validate_phone_endpoint(
    request: PhoneValidationRequest, session: AsyncSession = Depends(get_async_session)
):
    """
...
async def check_domain_mx_records(
    domain: str, session: AsyncSession = Depends(get_async_session)
):
    """
...
async def get_email_suggestions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Email suggestion generation failed: {e!s}"
        )
...
async def validation_health_check():
    ...
    try:
        ...
        dns_available = (
            email_validator.DNS_AVAILABLE
            if hasattr(email_validator, "DNS_AVAILABLE")
            else False
        )
