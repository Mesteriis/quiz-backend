...
class AdminSurveyCreate(BaseModel):
    ...
    questions: list["AdminQuestionCreate"] = Field(
        default_factory=list, description="Survey questions"
    )
...
class SurveyStats(BaseModel):
    ...
    average_time: Optional[float] = Field(
        default=None, description="Average completion time in seconds"
    )
    last_response: Optional[datetime] = Field(
        default=None, description="Last response timestamp"
    )
    daily_responses: list[dict[str, Any]] = Field(
        default_factory=list, description="Daily response counts"
    )
    question_stats: list[dict[str, Any]] = Field(
        default_factory=list, description="Per-question statistics"
    )
...
class AdminUserData(BaseModel):
    ...
    surveys_participated: list[str] = Field(
        ..., description="Survey titles participated in"
    )
...
class AdminAnalytics(BaseModel):
    ...
    top_surveys: list[dict[str, Any]] = Field(
        ..., description="Top surveys by response count"
    )
    user_countries: dict[str, int] = Field(
        ..., description="User distribution by country"
    )
    user_devices: dict[str, int] = Field(..., description="User distribution by device")
    user_browsers: dict[str, int] = Field(
        ..., description="User distribution by browser"
    )
...
class AdminReportRequest(BaseModel):
    ...
    include_user_data: bool = Field(
        default=True, description="Include user data in report"
    )
    include_raw_responses: bool = Field(
        default=True, description="Include raw responses"
    )
