...
class EmailValidationService:
    ...
    async def validate_email_comprehensive(
        ...
        try:
            ...
            result["is_valid"] = (
                ...
                (result["mx_valid"] if check_mx else True) and
                (result["smtp_valid"] if check_smtp else True)
            )
    ...
    async def _validate_format(self, email: str) -> Dict[str, any]:
        ...
        try:
            ...
            validation = validate_email(
                email,
                check_deliverability=False  # We'll do this separately
            )
            ...
            logger.debug(f"Email format invalid for {email}: {str(e)}")
            ...
            logger.error(f"Unexpected error in format validation: {str(e)}")
            result["error_message"] = f"Format validation error: {str(e)}"
    ...
    async def _validate_mx_records(self, domain: str) -> Dict[str, any]:
        ...
        try:
            ...
            logger.error(f"MX validation error for {domain}: {str(e)}")
            result["error_message"] = f"MX validation error: {str(e)}"
        ...
        for mx in mx_records:
            ...
            try:
                ...
                if smtp_result["success"]:
                    ...
                    logger.debug(f"SMTP test failed for {server}: {smtp_result['error']}")
                    
            except Exception as e:
                logger.debug(f"SMTP connection error to {server}: {str(e)}")
                continue
        ...
        def test_smtp():
            try:
                ...
                if code in [250, 251]:  # 250 = OK, 251 = User not local
                    return {"success": True, "response": response.decode() if isinstance(response, bytes) else response}
                else:
