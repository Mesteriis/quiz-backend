...
from services.jwt_service import jwt_service, create_user_token
...
class UserService:
    ...
    async def create_user(self, session: AsyncSession, user_data: UserCreate) -> Optional[User]:
        """Create a new user."""
        try:
            # Check if user already exists
            if user_data.telegram_id:
                existing_user = await self.get_user_by_telegram_id(session, user_data.telegram_id)
                if existing_user:
                    logger.warning(f"User with Telegram ID {user_data.telegram_id} already exists")
            ...
            user = User(
                ...
                is_verified=bool(user_data.telegram_id),  # Telegram users are auto-verified
    ...
    async def update_user(self, session: AsyncSession, user_id: int, user_data: UserUpdate) -> Optional[User]:
    ...
    async def authenticate_user(self, session: AsyncSession, identifier: str, telegram_id: Optional[int] = None) -> Optional[User]:
        """
    ...
    async def create_or_update_telegram_user(
        ...
        try:
            ...
            logger.error(f"Error creating/updating Telegram user {telegram_id}: {str(e)}")
            await session.rollback()
            return None
    ...
    async def get_user_from_token(self, session: AsyncSession, token: str) -> Optional[User]:
...
async def create_or_get_telegram_user(
    ...
    return await user_service.create_or_update_telegram_user(
        session, telegram_id, telegram_username, telegram_first_name, telegram_last_name, telegram_photo_url
    ) 