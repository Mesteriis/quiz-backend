...
class PDFService:
    ...
    def generate_survey_report(
        ...
        try:
            ...
            # Title
            title = Paragraph(f"Survey Report: {survey_data['title']}", self.styles['CustomTitle'])
            story.append(title)
            ...
            analytics_info = [
                ["Unique Respondents:", str(analytics_data.get('unique_respondents', 0))],
                ["Completion Rate:", f"{analytics_data.get('completion_rate', 0):.1f}%"],
                ["Authenticated Users:", str(analytics_data.get('authenticated_users', 0))],
            ...
            # Responses summary
            if responses_data:
                story.append(Paragraph("Responses Summary", self.styles['CustomHeading']))
                ...
                for question_id, data in questions_responses.items():
                    ...
                    story.append(Paragraph(f"Question: {question['title']}", self.styles['CustomSubheading']))
                    story.append(Paragraph(f"Type: {question['type']}", self.styles['CustomBody']))
                    story.append(Paragraph(f"Total Responses: {len(responses)}", self.styles['CustomBody']))
                    
                    # Show response samples
                    if responses:
                        story.append(Paragraph("Sample Responses:", self.styles['CustomBody']))
                        
                        sample_responses = responses[:5]  # Show first 5 responses
                        for i, response in enumerate(sample_responses, 1):
                            answer_text = self._format_answer(response['answer'])
                            story.append(Paragraph(f"{i}. {answer_text}", self.styles['CustomBody']))
                        
                        if len(responses) > 5:
                            story.append(Paragraph(f"... and {len(responses) - 5} more responses", self.styles['CustomCaption']))
            ...
            if responses_data:
                ...
                story.append(Paragraph("Detailed Responses", self.styles['CustomHeading']))
                
                # Create detailed responses table
                response_headers = ["#", "Question", "Answer", "User", "Date"]
                response_rows = [response_headers]
                
                for i, response in enumerate(responses_data[:100], 1):  # Limit to 100 responses
                    user_info = "Anonymous"
                    ...
                    response_rows.append([
                        str(i),
                        response['question']['title'][:30] + "..." if len(response['question']['title']) > 30 else response['question']['title'],
                        answer_text[:50] + "..." if len(answer_text) > 50 else answer_text,
                        user_info,
                        date_str
                    ])
                
                if len(response_rows) > 1:
                    response_table = Table(response_rows, colWidths=[0.5*inch, 2*inch, 2*inch, 1.5*inch, 1*inch])
                    ...
                    if len(responses_data) > 100:
                        ...
                        story.append(Paragraph(f"Note: Showing first 100 of {len(responses_data)} total responses", self.styles['CustomCaption']))
            
            # Footer
            story.append(Spacer(1, 30))
            story.append(Paragraph("Report generated by Quiz App", self.styles['CustomCaption']))
            story.append(Paragraph(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", self.styles['CustomCaption']))
    ...
    def generate_user_report(self, user_data: Dict[str, Any], user_responses: List[Dict[str, Any]]) -> bytes:
        """
        ...
        try:
            ...
            # Title
            title = Paragraph(f"User Report: {user_data.get('display_name', 'Unknown User')}", self.styles['CustomTitle'])
            story.append(title)
            story.append(Spacer(1, 20))
            ...
            if user_responses:
                ...
                for i, response in enumerate(user_responses, 1):
                    story.append(Paragraph(f"{i}. {response['question']['title']}", self.styles['CustomSubheading']))
                    story.append(Paragraph(f"Question Type: {response['question']['type']}", self.styles['CustomBody']))
                    
                    answer_text = self._format_answer(response['answer'])
                    story.append(Paragraph(f"Answer: {answer_text}", self.styles['CustomBody']))
                    
                    story.append(Paragraph(f"Responded on: {response['created_at']}", self.styles['CustomCaption']))
                    story.append(Spacer(1, 15))
...
def generate_survey_pdf_report(
    ...
    Returns:
        PDF bytes
    """
    return pdf_service.generate_survey_report(survey_data, responses_data, analytics_data)
