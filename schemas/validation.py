...
class EmailValidationResponse(BaseModel):
    ...
    suggestions: List[str] = Field(default_factory=list, description="Suggested corrections for typos")
...
class PhoneValidationResponse(BaseModel):
    ...
    country_code: Optional[str] = Field(None, description="Detected or provided country code")
...
class EmailBatchValidationRequest(BaseModel):
    ...
    emails: List[str] = Field(..., min_items=1, max_items=100, description="List of emails to validate")
    check_mx: bool = Field(True, description="Whether to check MX records")
    check_smtp: bool = Field(False, description="Whether to test SMTP connectivity")
...
class EmailBatchValidationResponse(BaseModel):
    ...
    results: List[EmailValidationResponse] = Field(..., description="Individual validation results")
...
class DomainMXResponse(BaseModel):
    ...
    mx_records: List[dict] = Field(default_factory=list, description="MX record details")
...
class ValidationHealthResponse(BaseModel):
    ...
    format_validation_working: bool = Field(..., description="Whether format validation is working")
...
class TelegramAuthRequest(BaseModel):
    ...
    username: str = Field(..., min_length=1, max_length=100, description="Telegram username")
...
class BrowserFingerprint(BaseModel):
    ...
    webgl_fingerprint: Optional[str] = Field(default=None, description="WebGL fingerprint")
...
class GeolocationData(BaseModel):
    ...
    altitude_accuracy: Optional[float] = Field(default=None, description="Altitude accuracy")
...
class UserSessionData(BaseModel):
    ...
    geolocation: Optional[GeolocationData] = Field(default=None, description="Geolocation data")
    telegram_data: Optional[TelegramWebAppData] = Field(default=None, description="Telegram data")
    referrer: Optional[str] = Field(default=None, description="Referrer URL")
    entry_page: str = Field(..., description="Entry page URL")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Session start time")
